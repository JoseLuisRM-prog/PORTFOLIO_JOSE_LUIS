...
```
Weight.kg y BMI estaban muy relacionadas, asi que se eliminó Weight.kg
...
ECHAMOS UN VISTAZO PARA VER QUE ESTÁ BIEN
```{r}
head(data_cleaned)
```
TODAS SE NOS HAN ESCALADO CORRECTAMENTE. AHORA QUEDAN LAS NOMINALES (SOLO ES DIABETES)
...
EMPEZAREMOS DESDE LA RELACION DE LAS VARIABLES, QUE ES CUANDO CAMBIAMO DE DATA A DATA_CLEANED Y SEGUIREMOS LOS MISMOS PASOS
```{r}
variables_numericas = data %>%
  select(where(is.numeric))
